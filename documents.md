...
|------|--------|----------|----------|------------|----------|
| 1 | P1 | gm 模拟账户订单生命周期联调 | `runner`, `execution` | 订单提交、受理、部分成交、完全成交、拒单、撤单和重复回报适配 | ✅ 代码适配与模拟回报测试完成；待交易时段执行一次真实模拟账户链路 |
...
| 阶段 | 测试任务 | 当前结果 | 待补验证 |
|------|----------|----------|----------|
| P0 阶段1 | `execution` 基础功能与 API 测试 | ✅ 18 个通过 | Order 真实交易回报集成测试 |
...
| P1 阶段1 | 交易安全闭环单元与跨模块测试 | ✅ 92 个通过（execution + plans + runner 联合回归） | 模拟账户交易时段真实链路、并发资金扣减、告警通道 |
| 阶段6 | 全项目回归测试 | ⚠️ 文档曾记录 136 个通过；该数字包含历史测试集合，不能与模块测试数直接相加 | 重新执行完整回归并统一统计口径 |
