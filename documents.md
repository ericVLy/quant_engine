...
|--------|----------|----------|--------------|
| P1 | 扩展真实行情、基本面和实时快照数据上下文 | `runner`, `datasources` | R-07、D-02、D-09 |
| P1 | 接入真实因子计算、过滤、裁决逻辑 | `cases`, `runner` | C-09、R-06 |
| P1 | 完善复杂仓位、资金和交易时段风控 | `runner`, `execution` | R-08 |
| P1 | 完善真实交易环境回报字段和订单生命周期适配 | `runner`, `execution` | EX-18 |
| P1 | 实现 Case 参数 JSON Schema 和版本历史 | `cases` | C-07、C-09 |
| P1 | 实现 Suite 条件路由、运行时聚合和并行执行 | `suites`, `execution` | S-09、S-10、S-11、EX-15 |
| P1 | 完善 Plan Cron、版本快照、执行模式和重试策略 | `plans`, `runner` | P-03、P-05、P-08、P-09、P-10、R-09 |

#### 5.1.3 开发顺序

| 顺序 | 开发阶段 | 主要交付物 | 关联需求 |
|------|----------|------------|----------|
| 1 | `cases` 剩余能力 | 参数 Schema、版本历史、真实 CaseExecutor | C-07、C-09、R-06 |
| 2 | `suites` 剩余能力 | 条件路由、运行时聚合、并行节点执行 | S-09、S-10、S-11、EX-15 |
| 3 | `plans` 剩余能力 | 完整 Cron、版本快照、执行模式、热加载 | P-03、P-05、P-08、P-09、P-10、R-09 |
| 4 | `runner` 生产能力 | 数据上下文、复杂风控、真实交易环境适配 | R-06、R-07、R-08、EX-18 |
...
| 阶段 | 测试任务 | 当前结果 | 待补验证 |
|------|----------|----------|----------|
| 阶段1 | `execution` 基础功能与 API 测试 | ✅ 12 个通过 | Order 真实交易回报集成测试 |
| 阶段2 | `cases` CRUD、发布和触发校验测试 | ✅ 8 个通过 | Schema、版本历史、真实计算测试 |
| 阶段3 | `suites` CRUD、拓扑和 DAG 测试 | ✅ 9 个通过 | 条件路由、运行时聚合、并行执行测试 |
| 阶段4 | `plans` CRUD、发布和标的解析测试 | ✅ 10 个通过 | 完整 Cron、版本快照、重试策略测试 |
| 阶段5 | `runner`、gm SDK 和 execution 联动测试 | ✅ 8 个通过 | 生产行情、风控边界、真实交易环境测试 |
...
| N-03 | 策略配置变更支持热加载（无需重启服务） | P0；关联开发任务：5.1.2-7；关联测试任务：5.2-7 |
