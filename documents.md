...
| `cases` | 🟡 P0 基础能力完成 | 8 通过 | 85%（复杂 Schema 规则和真实因子引擎待完善） |
| `suites` | 🟡 P0 基础能力完成 | 9 通过 | 85%（跨 Suite 运行时编排和完整画布拓扑待完善） |
| `plans` | 🟡 P0 基础能力完成 | 10 通过 | 85%（持久化调度进程和版本回滚待完善） |
...
| P0 阶段2 | `cases` CRUD、发布和触发校验测试 | ✅ 8 个通过 | Schema、版本历史、真实计算测试 |
| P0 阶段3 | `suites` CRUD、拓扑和 DAG 测试 | ✅ 9 个通过 | 条件路由、运行时聚合、并行执行测试 |
| P0 阶段4 | `plans` CRUD、发布和标的解析测试 | ✅ 10 个通过 | 完整 Cron、版本快照、重试策略测试 |
| P0 阶段5 | `runner`、gm SDK 和 execution 联动测试 | ✅ 8 个通过 | 生产行情、风控边界、真实交易环境测试 |
