...
- 周期、MACD 参数和权重等数值字段会校验类型、有限性和最小值，并拒绝布尔值伪装成整数。
...
|------|----------|------------|----------|------|
| 1 | P0 阻塞：`cases` 剩余能力 | 参数 Schema、版本历史、真实 CaseExecutor | C-07、C-09、R-06 | 🟢 基础能力完成；指标目录和更深层语义校验待补 |
...
| P0 阶段2 | `cases` CRUD、发布和深层参数校验测试 | ✅ 16 个通过 | 指标目录和更深层语义校验 |
