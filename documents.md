...
| P0 | 持久化 Cron 调度、配置刷新、重试策略和版本回滚 | `plans`, `runner` | P-03、P-05、P-08、P-09、P-10、R-01、R-09 |
| P0 | 完成 Case 参数 JSON Schema 深层校验（指标目录与跨字段语义规则） | `cases` | C-07、C-09 |
...
|------|----------|------------|----------|------|
| 1 | P0 阻塞：`cases` 剩余能力 | 参数 Schema、版本历史、真实 CaseExecutor | C-07、C-09、R-06 | ✅ 已完成（结构白名单、指标目录、跨字段语义校验） |
| 2 | P0 阻塞：`suites` 剩余能力 | 条件路由、运行时聚合、并行节点执行 | S-09、S-10、S-11、EX-15 | ✅ 已完成（2026-09-04：SuiteVersion 快照 + NodeRun + 递归编排） |
| 3 | P0 阻塞：`plans` 剩余能力 | 持久化 Cron、配置刷新、执行模式和版本管理 | P-03、P-05、P-08、P-09、P-10、R-09 | ✅ 已完成（Cron、热刷新、重试策略、版本回滚） |
